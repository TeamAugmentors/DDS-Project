...
BEGIN
	...
	select COUNT(*) into cnt from vaccine_center@server where city = 'Sylhet';
	j := trunc(dbms_random.value(1, cnt + 1));

	i:= 1;
	FOR R IN (SELECT CID FROM VACCINE_CENTER@server WHERE city = 'Sylhet') LOOP
	...
	--insert into users@server values(:new.nid, :new.name, 'Sylhet', random_vaccine_no, random_center_no);
